...
void ProgramLoop::run() {
	...
	for (int i = 0; i < driversData.getObjectNumber(); i++) {
		...
		if (driversData.read(i, 0).length() != 14) {
			driversData.deletePatch(i);
			std::cout << "Invalid Data found on database, deleted" << std::endl;
			continue;
		}
		if (driversData.read(i, 4).length() != 10) {
			driversData.deletePatch(i);
			std::cout << "Invalid Data found on database, deleted" << std::endl;
			continue;
		}
	...
	while (running) {
		...
		if (command == "rent") {
			...
			std::cout << "the rented cars is :" << endl;
			int index = 1;
			for (int i = 0; i < cars.size(); i++) {
				if (cars[i].get_renter() != "not rented") {
					std::cout << index << ". model: " << std::setw(20) << cars[i].get_car_model() << std::setw(10) << " , Owner: " << cars[i].get_owner() << " , plate number: " << cars[i].get_license_plates_number() << std::endl;
					index++;
				}
			}
		}
